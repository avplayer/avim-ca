...
ca_service::ca_service(io_service_pool& ios, unsigned short port, std::string address /*= "0.0.0.0"*/)
...
void ca_service::start()
{
	start_impl();
	continue_timer();
}

void ca_service::start_impl()
{
	m_connection = boost::make_shared<connection>(boost::ref(m_io_service_pool.get_io_service()), boost::ref(*this), &m_connection_manager);
	m_acceptor.async_accept(m_connection->socket(), boost::bind(&ca_service::handle_accept, this, boost::asio::placeholders::error));
}

void ca_service::stop()
{
	boost::system::error_code ignore_ec;
...
void ca_service::handle_accept(const boost::system::error_code& error)
{
...
void ca_service::on_tick(const boost::system::error_code& error)
{
	// TODO: 定时要做的事.
	continue_timer();
}

bool ca_service::do_message(google::protobuf::Message* msg, connection_ptr conn)
{
...
void ca_service::do_connection_notify(int type, connection_ptr conn)
{
...
bool ca_service::add_message_process_moudle(const std::string& name, message_callback cb)
{
...
bool ca_service::del_message_process_moudle(const std::string& name)
{
	boost::unique_lock<boost::shared_mutex> l(m_message_callback_mtx);
...
bool ca_service::add_connection_process_moudle(const std::string& name, connection_callback cb)
{
...
bool ca_service::del_connection_process_moudle(const std::string& name)
{
	boost::unique_lock<boost::shared_mutex> l(m_connection_callback_mtx);
...
void ca_service::continue_timer()
{
	m_timer.expires_from_now(seconds(1));
	m_timer.async_wait(boost::bind(&ca_service::on_tick, this, boost::asio::placeholders::error));
}
