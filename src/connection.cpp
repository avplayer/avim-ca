...
connection::connection(boost::asio::io_service& io, ca_service& serv, connection_manager* connection_man)
	: m_io_service(io)
	, m_server(serv)
	, m_socket(io)
	...
	if (!error)
	{
		...
		if (!m_write_queue.empty())
		{
			boost::asio::async_write(m_socket,
				boost::asio::buffer(m_write_queue.front().msg.data(), m_write_queue.front().msg.length()),
